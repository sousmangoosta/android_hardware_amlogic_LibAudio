...
void *audio_decode_loop_omx(void *args)
{
    ...
    rawoutput_enable = audec->raw_enable;
   adec_print(" zll rawoutput_enable/%d", rawoutput_enable);
    while (1) {
//exit_decode_loop:
        ...
        if (g_bst) {
            int wlen = 0;
            if (rawoutput_enable == 0) {
                while ((outlen > 0) && !audec->exit_decode_thread) {
                    if ((g_bst->buf_length - g_bst->buf_level) < outlen) {
                        amthreadpool_thread_usleep(20000);
                        continue;
                    }
                    wlen = write_pcm_buffer(outbuf, g_bst, outlen);
                    //adec_print("[%s %d] ----write_pcm_buffer !!!! wlen:%d, outlen:%d\n", __FUNCTION__, __LINE__, wlen, outlen);
                    outlen -= wlen;
                    audec->pcm_cache_size -= wlen;
                }
            } else {
                while (rawoutput_enable && !audec->exit_decode_thread && outlen_raw) {
                    //adec_print("left %d",g_bst_raw->buf_length - g_bst_raw->buf_level);
                    if (g_bst_raw->buf_length - g_bst_raw->buf_level < outlen_raw) {
                        amthreadpool_thread_usleep(20000);
                        continue;
                    }
                    wlen = write_pcm_buffer(outbuf, g_bst_raw, outlen_raw);
                    //adec_print("[%s %d] ----write_raw_buffer !!!! wlen:%d, outlen:%d\n", __FUNCTION__, __LINE__, wlen, outlen);
                    if ( audec->format == ACODEC_FMT_AC3)
                        audec->pcm_bytes_readed = audec->pcm_bytes_readed + wlen;
                    else if (audec->format == ACODEC_FMT_EAC3)
                        audec->pcm_bytes_readed = audec->pcm_bytes_readed + wlen/4;
                    outlen_raw -= wlen;
                }
            }
        }
...
void stop_decode_thread_omx(aml_audio_dec_t *audec)
{
    if (audec->sn_threadid == -1) {
        adec_print("[%s] need return!!! thread have not create yet.\n", __FUNCTION__);
        return;
    }
    audec->exit_decode_thread = 1;
